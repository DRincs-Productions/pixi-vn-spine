...
export default class Spine extends Container<CoreSpine & CanvasBaseItem<any>, SpineMemory> {
    ...
    get memory(): SpineMemory {
        return {
            ...
            state: {
                tracks: this.spine.state.tracks.map((track) => {
            ...
            currentSkin: this.spine.skeleton.skin?.name,
    ...
    addAnimation(
        ...
        return this.spine.state.addAnimation(trackIndex, animationName, loop, milliDelay);
    }
    /**
    ...
    private setTrackSequence(
        ...
        sequence.forEach(([currentAnimationName, animOptions]) => {
            const currentAnimation = this.spine.skeleton.data.findAnimation(currentAnimationName);
            if (!currentAnimation) {
...
async function setMemorySpine(element: any, memory: SpineMemory) {
    ...
    await setMemoryContainer(element, memory, {
            ...
            memory.autoUpdate !== undefined && (element.spine.autoUpdate = memory.autoUpdate);
    ...
    memory.state.tracks.forEach((track) => {
        ...
        element.spine.state.addAnimation(track.trackIndex, track.animationName, track.loop, track.delay);
    });
}
